...
```shell script
gradle compileJava quarkus:dev
```
...
```shell script
gradle assemble -Dquarkus.package.jar.type=uber-jar
```
...
```shell script
gradle assemble -Pnative
```

Or, if you don't have GraalVM installed, you can run the native executable build in a container using: 
```shell script
gradle assemble -Pnative -Dquarkus.native.container-build=true
```
...
```shell
$ gradle clean quarkus:dev -Dquarkus.http.port=8181
```
